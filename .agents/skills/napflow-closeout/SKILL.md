...
- **Session**: Use for state-changing work, a PR-sized change, or a useful handoff that does not complete a `docs/PLAN.md` milestone.
...
5. Distinguish completed work from planned work. Do not claim a commit, passing test, owner decision, or resolved issue without evidence.
...
| --- | --- |
| `docs/JOURNAL.md` | Prepend one dated entry for a Session or Milestone closeout. Keep it to 2–5 concise bullets covering done, decided when applicable, verification or blockers, and next. Do not create an entry in No update mode. |
...
3. Include a concrete next action that lets a fresh agent resume without reconstructing the session.
4. Mention uncommitted work or blockers accurately. Include a commit SHA only when that commit exists.
...
5. Do not commit, push, tag, publish, or open a pull request unless the user separately requested it.
