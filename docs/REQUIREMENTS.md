...
- [x] NFR-10 Dependency-compat CI job: install napflow into a venv alongside `requests` + `botocore` and run the test suite — guards the niquests/urllib3-future conflict class in users' pytest envs. (review 2026-07-02) — `compat` job in ci.yml (uv venv + requests + botocore alongside the wheel, suite re-run), 2026-07-05
