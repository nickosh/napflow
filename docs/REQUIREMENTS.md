...
- [ ] FR-405 (S2) Outcome aggregation: asserts, python-asserts, unhandled error-port messages roll up run-wide; run state = worst outcome in the frame tree. (D20) — root-frame aggregation incl. python-asserts at S3/M2 (`test_wired_python_assert_still_fails_run`); the frame-tree half completes at S3/M5
